<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Conversazione Richiesta Noleggio Auto da {{ $request->nome }} {{ $request->cognome }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 flex justify-between items-center border-b pb-4">
                        ...
                        <a href="{{ route('admin.car-rentals.index') }}" class="text-indigo-600 hover:text-indigo-900 font-semibold text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div>
                            <h3 class="text-sm font-bold tracking-widest text-gray-400 uppercase mb-2">Dettagli Noleggio</h3>
                            <ul class="text-gray-700 bg-indigo-50/50 p-4 rounded-lg border border-indigo-200 text-sm">
                                <li class="mb-1"><strong class="font-semibold w-32 inline-block">Ritiro:</strong> {{ \Carbon\Carbon::parse($request->data_ritiro)->format('d/m/Y') }} alle {{ $request->orario_ritiro }}</li>
                                <li class="mb-1"><strong class="font-semibold w-32 inline-block">Riconsegna:</strong> {{ \Carbon\Carbon::parse($request->data_riconsegna)->format('d/m/Y') }} alle {{ $request->orario_riconsegna }}</li>
                                <li><strong class="font-semibold w-32 inline-block">Posti:</strong> {{ $request->numero_posti }}</li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div class="border border-gray-200 rounded-lg p-6 space-y-6 max-h-[500px] overflow-y-auto bg-gray-50/50">
                            ...
                            <div class="flex items-start space-x-3 max-w-[85%]">
                                ...
                                <div class="bg-indigo-50 border border-indigo-100/50 rounded-2xl rounded-tl-none p-4 text-gray-800 shadow-sm text-sm">
                                    ...
                                    <p class="whitespace-pre-line">{{ $request->messaggio ?? 'Richiesta di noleggio auto inviata.' }}</p>
                                </div>
                            </div>
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 mb-8">
                        ...
                        <form action="{{ route('admin.car-rentals.reply', $request) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-4">
                    ...
                    <div class="mt-8 pt-4 border-t flex justify-between items-center">
                        ...
                        <form action="{{ route('admin.car-rentals.destroy', $request) }}" method="POST" onsubmit="return confirm('Sei sicuro di voler eliminare definitivamente questa richiesta?');">
                            @csrf
                            @method('DELETE')
