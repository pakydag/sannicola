                        ...
                        <table class="min-w-full bg-white border border-gray-200">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th class="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Info Noleggio</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse($requests as $richiesta)
                                    <tr class="hover:bg-gray-50 {{ !$richiesta->letto ? 'bg-indigo-50 font-semibold' : '' }}">
                                        ...
                                        <td class="py-4 px-6 text-sm text-gray-500">
                                            <strong>Da:</strong> {{ \Carbon\Carbon::parse($richiesta->data_ritiro)->format('d/m/Y') }} {{ $richiesta->orario_ritiro }}<br>
                                            <strong>A:</strong> {{ \Carbon\Carbon::parse($richiesta->data_riconsegna)->format('d/m/Y') }} {{ $richiesta->orario_riconsegna }}<br>
                                            <span class="text-xs text-gray-600">{{ $richiesta->numero_posti }} posti</span>
                                        </td>
                                        <td class="py-4 px-6 text-sm text-right space-x-2">
                                            <a href="{{ route('admin.car-rentals.show', $richiesta) }}" class="text-indigo-600 hover:text-indigo-900">Leggi</a>
                                            
                                            <form action="{{ route('admin.car-rentals.destroy', $richiesta) }}" method="POST" class="inline-block" onsubmit="return confirm('Sei sicuro di voler eliminare questa richiesta?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-4 px-6 text-sm text-center text-gray-500">
                                            Nessuna richiesta di noleggio ricevuta finora.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
