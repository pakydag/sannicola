                        ...
                        <table class="min-w-full bg-white border border-gray-200">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th class="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email / Telefono</th>
                                    <th class="py-3 px-6 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Azione</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                @forelse($contactRequests as $richiesta)
                                    <tr class="hover:bg-gray-50 {{ !$richiesta->letto ? 'bg-indigo-50 font-semibold' : '' }}">
                                        ...
                                        <td class="py-4 px-6 text-sm text-gray-900">
                                            ...
                                            @if($richiesta->ragione_sociale)
                                                <br><span class="text-xs text-gray-500">{{ $richiesta->ragione_sociale }}</span>
                                            @endif
                                        </td>
                                        <td class="py-4 px-6 text-sm text-gray-500">
                                            <a href="mailto:{{ $richiesta->email }}" class="text-indigo-600 hover:text-indigo-900">{{ $richiesta->email }}</a>
                                            @if($richiesta->telefono)
                                                <br>{{ $richiesta->telefono }}
                                            @endif
                                        </td>
                                        <td class="py-4 px-6 text-sm text-right space-x-2">
                                            <a href="{{ route('admin.contatti.show', $richiesta) }}" class="text-indigo-600 hover:text-indigo-900">Leggi</a>
                                            
                                            <form action="{{ route('admin.contatti.destroy', $richiesta) }}" method="POST" class="inline-block" onsubmit="return confirm('Sei sicuro di voler eliminare questa richiesta?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-4 px-6 text-sm text-center text-gray-500">
                                            Nessuna richiesta di contatto ricevuta finora.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
