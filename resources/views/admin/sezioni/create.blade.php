        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 border-b border-gray-200">
                    ...
                    <form action="{{ route('admin.sezioni.store') }}" method="POST">
                        @csrf
                        ...
                        @if($englishEnabled)
                            <div class="mb-4 bg-indigo-50/30 p-3 rounded border border-indigo-100/50">
                                ...
                                <input type="text" name="nome_en" id="nome_en" value="{{ old('nome_en') }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="Lascia vuoto per autogenerare (es. 5-it)"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="ordine" id="ordine" value="{{ old('ordine', 0) }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex items-center gap-6 mt-2">
                                <label class="flex items-center">
                                    <input type="radio" name="tipo" value="pagina" class="form-radio h-4 w-4 text-indigo-600 transition duration-150 ease-in-out" {{ old('tipo') == 'pagina' ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm text-gray-700"><strong>Pagina</strong> (Mostra solo il testo, nessun articolo)</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="radio" name="tipo" value="archivio" class="form-radio h-4 w-4 text-indigo-600 transition duration-150 ease-in-out" {{ old('tipo', 'archivio') == 'archivio' ? 'checked' : '' }}>
                        ...
                        <div class="mb-4" id="dropdown-wrapper">
                            <label class="flex items-center">
                                <input type="checkbox" name="menu_a_tendina" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('menu_a_tendina', false) ? 'checked' : '' }}>
                        ...
                        <div class="mb-6" id="grid-columns-wrapper">
                            ...
                            <select name="colonne_griglia" id="colonne_griglia" class="shadow border rounded w-full md:w-1/4 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                <option value="1" {{ old('colonne_griglia', 3) == 1 ? 'selected' : '' }}>1 Articolo (Lista singola)</option>
                                <option value="2" {{ old('colonne_griglia', 3) == 2 ? 'selected' : '' }}>2 Articoli affiancati</option>
                                <option value="3" {{ old('colonne_griglia', 3) == 3 ? 'selected' : '' }}>3 Articoli affiancati</option>
                                <option value="4" {{ old('colonne_griglia', 3) == 4 ? 'selected' : '' }}>4 Articoli affiancati</option>
                                <option value="6" {{ old('colonne_griglia', 3) == 6 ? 'selected' : '' }}>6 Articoli affiancati</option>
                            </select>
                        ...
                        @if($englishEnabled)
                            <div class="mb-8 bg-indigo-50/30 p-3 rounded border border-indigo-100/50">
                                <div class="flex items-center justify-between mb-2">
                                    <label for="contenuto_en" class="block text-indigo-950 text-sm font-bold">Contenuto [INGLESE] (Testo o HTML)</label>
                                    <button type="button" id="fm-button-en" class="bg-gray-200 hover:bg-gray-300 text-gray-800 text-xs font-bold py-1 px-3 rounded inline-flex items-center">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                        Inserisci Immagine dal File Manager [EN]
                                    </button>
                                </div>
                                <textarea name="contenuto_en" id="contenuto_en" rows="8"
                                    class="shadow appearance-none border border-indigo-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline font-mono text-sm bg-white">{{ old('contenuto_en') }}</textarea>
                        ...
                        <div class="mt-10 mb-8 border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
                            ...
                            <div class="p-6">
                                ...
                                @if($englishEnabled)
                                    <div class="mb-4 bg-indigo-50/30 p-3 rounded border border-indigo-100/50">
                                        <label for="seo_title_en" class="block text-indigo-955 text-sm font-bold mb-2">Titolo per Motori di Ricerca/Social [INGLESE]</label>
                                        <input type="text" name="seo_title_en" id="seo_title_en" value="{{ old('seo_title_en') }}" placeholder="Custom title in English..."
                                            class="shadow appearance-none border border-indigo-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-white">
                                ...
                                @if($englishEnabled)
                                    <div class="mb-4 bg-indigo-50/30 p-3 rounded border border-indigo-100/50">
                                        <label for="seo_description_en" class="block text-indigo-955 text-sm font-bold mb-2">Descrizione/Riassunto Social [INGLESE] (max 160 caratteri)</label>
                                        <textarea name="seo_description_en" id="seo_description_en" rows="3"
                                            class="shadow appearance-none border border-indigo-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-white"
                                            placeholder="Enter English SEO description...">{{ old('seo_description_en') }}</textarea>
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                function toggleArchivioOptions() {
                    if (radioArchivio.checked) {
                        ...
                        // Optionally uncheck when hiding to avoid accidental saves
