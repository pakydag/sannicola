...
<div class="overflow-hidden bg-white hover:shadow-2xl transition-shadow duration-300 flex flex-col h-full border border-gray-100 rounded-lg">
    ...
    <div class="titoli p-6 flex-1 flex flex-col">
        ...
        @if($s->prenotabile)
            <div class="flex items-center gap-4 text-gray-500 text-sm mb-4 font-normal">
                <span class="flex items-center gap-1">
                    ...
                    {{ $s->camere_letto }} {{ app()->getLocale() === 'en' ? 'Bedrooms' : 'Camere' }}
                </span>
                <span class="flex items-center gap-1">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    {{ $s->posti_totali }} {{ app()->getLocale() === 'en' ? 'Guests' : 'Ospiti' }}
                </span>
            </div>
        @endif
        ...
        <div class="mt-auto">
            @if($s->prenotabile)
                <a href="{{ route('public.booking.show', ['id' => $s->id, 'slug' => \Illuminate\Support\Str::slug($s->nome)]) }}" class="btn mb-5 rounded-md">
