...
<div class="bg-white shadow-sm xl:p-14 p-8 overflow-hidden relative rounded-lg" id="contatti">
    <div class="text-center mb-8 hidden">
        <h3 class="text-2xl font-bold text-gray-900 mb-2">{{ $isEn ? 'Contact Us' : 'Contattaci' }}</h3>
        <p class="text-gray-600">{{ $isEn ? 'Fill in the form below to send us a request. We will reply as soon as possible.' : 'Compila il modulo sottostante per inviarci una richiesta. Ti risponderemo al più presto.' }}</p>
    </div>
    ...
    @else
        <form action="{{ route('public.contact.submit') }}#contatti" method="POST" class="max-w-4xl mx-auto">
            ...
            <div class="mb-6">
                <label for="ragione_sociale" class="block text-sm font-medium text-gray-700 mb-1">{{ $isEn ? 'Company Name (Optional)' : 'Ragione Sociale (Opzionale)' }}</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <!-- Email -->
                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700 mb-1">{{ $isEn ? 'Email *' : 'E-mail *' }}</label>
            ...
            <div class="mb-6">
                <label for="richiesta" class="block text-sm font-medium text-gray-700 mb-1">{{ $isEn ? 'Request *' : 'Richiesta *' }}</label>
