...
<div class="max-w-7xl mx-6 lg:mx-auto py-6 px-11 sm:px-12 lg:px-14 bg-white shadow-sm rounded-b-lg mb-12 relative z-10">
    <header class="titoli text-center mb-5">
        <h1 class="text-3xl sm:text-4xl tracking-tight primary">
            {{ $sezione->nome }}
        </h1>
                    ...
                    @foreach($articoli as $articolo)
                        <div class="bg-white rounded-lg shadow-sm overflow-hidden flex flex-col transition hover:shadow-md">
                            ...
                            <div class="pb-6 px-6 text-center">
                                <a href="{{ route('public.articolo', ['sezione_slug' => $sezione->slug ?? $sezione->id.'-it', 'articolo_slug' => $articolo->slug ?? $articolo->id.'-it']) }}" class="btn mb-5 rounded-md">{{ app()->getLocale() === 'en' ? 'Read more' : 'Leggi di più' }} &rarr;</a>
                            </div>
                        </div>
                    @endforeach
                </div>
