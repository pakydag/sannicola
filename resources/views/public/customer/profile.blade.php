        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-x-8">
            ...
            <aside class="lg:col-span-3 mb-8 lg:mb-0">
                <nav class="space-y-1 bg-white p-4 rounded-lg shadow">
                    ...
                    <a href="{{ route('public.account.profile') }}" class="bg-gray-100 text-primary hover:text-primary hover:bg-gray-200 group rounded-md px-3 py-2 flex items-center text-sm font-medium">
                        <svg class="text-primary group-hover:text-primary flex-shrink-0 -ml-1 mr-3 h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="lg:col-span-9">
                <form action="{{ route('public.account.updateProfile') }}" method="POST" class="bg-white shadow rounded-lg px-4 py-6 sm:p-6 lg:p-8 space-y-6">
                    ...
                    <h2 class="text-xl font-semibold text-primary border-b border-gray-200 pb-4 mb-6">Dati Profilo e Spedizione</h2>
                    ...
                    <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="nome" name="nome" value="{{ old('nome', $customer->nome ?? (explode(' ', $user->name)[0] ?? '')) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                        </div>
                        <div>
                            <label for="cognome" class="block text-sm font-medium text-gray-700">Cognome *</label>
                            <input type="text" id="cognome" name="cognome" value="{{ old('cognome', $customer->cognome ?? (count(explode(' ', $user->name))>1 ? explode(' ', $user->name)[1] : '')) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" id="telefono" name="telefono" value="{{ old('telefono', $customer->telefono) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" id="indirizzo" name="indirizzo" value="{{ old('indirizzo', $customer->indirizzo) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                        </div>

                        <div>
                            <label for="citta" class="block text-sm font-medium text-gray-700">Città *</label>
                            <input type="text" id="citta" name="citta" value="{{ old('citta', $customer->citta) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                        </div>

                        <div>
                            <label for="cap" class="block text-sm font-medium text-gray-700">CAP *</label>
                            <input type="text" id="cap" name="cap" value="{{ old('cap', $customer->cap) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                        </div>
                        ...
                        <div class="sm:col-span-2 mt-6">
                            <div class="relative flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="is_azienda" name="is_azienda" type="checkbox" value="1" x-model="isAzienda" class="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded">
                                </div>
                        ...
                        <template x-if="isAzienda">
                            <div class="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4 bg-gray-100 p-4 rounded-md border border-gray-100">
                                <div class="sm:col-span-2">
                                    <label for="ragione_sociale" class="block text-sm font-medium text-gray-700">Ragione Sociale *</label>
                                    <input type="text" id="ragione_sociale" name="ragione_sociale" value="{{ old('ragione_sociale', $customer->ragione_sociale) }}" x-bind:required="isAzienda" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                                </div>
                                <div>
                                    <label for="partita_iva" class="block text-sm font-medium text-gray-700">Partita IVA *</label>
                                    <input type="text" id="partita_iva" name="partita_iva" value="{{ old('partita_iva', $customer->partita_iva) }}" x-bind:required="isAzienda" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                                </div>
                                <div>
                                    <label for="codice_fiscale" class="block text-sm font-medium text-gray-700">Codice Fiscale (Opzionale)</label>
                                    <input type="text" id="codice_fiscale" name="codice_fiscale" value="{{ old('codice_fiscale', $customer->codice_fiscale) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                                </div>
                                <div>
                                    <label for="sdi" class="block text-sm font-medium text-gray-700">Codice SDI</label>
                                    <input type="text" id="sdi" name="sdi" value="{{ old('sdi', $customer->sdi) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm placeholder-gray-400" placeholder="Es. KRRH6B9">
                                </div>
                                <div>
                                    <label for="pec" class="block text-sm font-medium text-gray-700">PEC Aziendale</label>
                                    <input type="email" id="pec" name="pec" value="{{ old('pec', $customer->pec) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm">
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="sm:col-span-2 mt-6 pt-6 border-t border-gray-100">
                             ...
                             <select id="metodo_pagamento_preferito" name="metodo_pagamento_preferito" class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                    ...
                    <div class="pt-5 border-t border-gray-200 flex justify-end">
                        <button type="submit" class="bg-gray-200 border border-transparent rounded-md shadow-sm py-2 px-6 text-sm font-medium text-white hover:bg-primary focus:outline-none focus:ring-1 focus:ring-offset-2 focus:ring-primary">
                            Salva Modifiche
                        </button>
                    </div>
                </form>
