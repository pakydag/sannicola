    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-t-xl shadow-md border-b p-6 md:p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 mb-2">
                        Rental Request #{{ str_pad($carRentalRequest->id, 5, '0', STR_PAD_LEFT) }}
                    </span>
                    <h1 class="text-2xl md:text-3xl font-serif text-gray-900 font-bold mb-1">
                        {{ $isEn ? 'Rental Thread' : 'Conversazione Richiesta Noleggio' }}
                    </h1>
                    <p class="text-gray-500 text-sm">
                        {{ $isEn ? 'Opened on' : 'Iniziata il' }} {{ $carRentalRequest->created_at->format('d/m/Y H:i') }} | 
                        <strong>{{ $carRentalRequest->nome }} {{ $carRentalRequest->cognome }}</strong>
                    </p>
                </div>
            ...
            <div class="mt-6 bg-gray-50 rounded-xl p-4 border border-gray-100 text-sm grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <h3 class="font-bold text-gray-800 mb-2">{{ $isEn ? 'Rental Details' : 'Dettagli di Noleggio' }}</h3>
                    <p class="text-gray-600 mb-1"><span class="font-semibold text-gray-800">{{ $isEn ? 'Pick Up' : 'Data Ritiro' }}:</span> {{ \Carbon\Carbon::parse($carRentalRequest->data_ritiro)->format('d/m/Y') }} @ {{ $carRentalRequest->orario_ritiro }}</p>
                    <p class="text-gray-600"><span class="font-semibold text-gray-800">{{ $isEn ? 'Drop Off' : 'Data Riconsegna' }}:</span> {{ \Carbon\Carbon::parse($carRentalRequest->data_riconsegna)->format('d/m/Y') }} @ {{ $carRentalRequest->orario_riconsegna }}</p>
                </div>
                <div>
                    <h3 class="font-bold text-gray-800 mb-2">{{ $isEn ? 'Seats' : 'Numero Posti' }}</h3>
                    <p class="text-gray-600"><span class="font-semibold text-gray-800">{{ $isEn ? 'Seats Requested' : 'Posti richiesti' }}:</span> {{ $carRentalRequest->numero_posti }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow-md p-6 md:p-8 space-y-6 max-h-[500px] overflow-y-auto border-b">
            ...
            <div class="flex items-start space-x-3 max-w-[85%]">
                <div class="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-sm shadow">
                    {{ strtoupper(substr($carRentalRequest->nome, 0, 1)) }}
                </div>
                <div class="bg-indigo-50 border border-indigo-100/50 rounded-2xl rounded-tl-none p-4 text-gray-800 shadow-sm text-sm">
                    <div class="flex items-center justify-between mb-1">
                        <span class="font-bold text-indigo-955">{{ $carRentalRequest->nome }} {{ $carRentalRequest->cognome }}</span>
                        <span class="text-[10px] text-indigo-500 ml-4">{{ $carRentalRequest->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <p class="whitespace-pre-line">{{ $carRentalRequest->messaggio ?: ($isEn ? 'Requested a car rental service.' : 'Richiesta servizio noleggio auto.') }}</p>
                </div>
            </div>

            <!-- Lista dei messaggi del Thread -->
            @foreach($carRentalRequest->messages as $msg)
                @if($msg->sender === 'admin')
        ...
        <div class="bg-white rounded-b-xl shadow-md p-6 md:p-8">
            ...
            <form action="{{ route('public.car_rental.thread.reply', ['token' => $carRentalRequest->secure_token]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
