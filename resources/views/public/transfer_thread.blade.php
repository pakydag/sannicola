...
<div class="py-16 bg-gray-50 flex-grow">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-t-xl shadow-md border-b p-6 md:p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 mb-2">
                        Transfer Request #{{ str_pad($transferRequest->id, 5, '0', STR_PAD_LEFT) }}
                    </span>
                    <h1 class="text-2xl md:text-3xl font-serif text-gray-900 font-bold mb-1">
                        {{ $isEn ? 'Transfer Thread' : 'Conversazione Richiesta Transfer' }}
                    </h1>
                    <p class="text-gray-500 text-sm">
                        {{ $isEn ? 'Opened on' : 'Iniziata il' }} {{ $transferRequest->created_at->format('d/m/Y H:i') }} | 
                        <strong>{{ $transferRequest->nome }} {{ $transferRequest->cognome }}</strong>
                    </p>
                </div>
            ...
            <div class="mt-6 bg-gray-50 rounded-xl p-4 border border-gray-100 text-sm grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <h3 class="font-bold text-gray-800 mb-2">{{ $isEn ? 'Stay & Travel Details' : 'Dettagli di Viaggio' }}</h3>
                    <p class="text-gray-600 mb-1"><span class="font-semibold text-gray-800">{{ $isEn ? 'Destination' : 'Luogo di Arrivo' }}:</span> {{ $transferRequest->luogo_arrivo }}</p>
                    <p class="text-gray-600 mb-1"><span class="font-semibold text-gray-800">{{ $isEn ? 'Date' : 'Data Arrivo' }}:</span> {{ \Carbon\Carbon::parse($transferRequest->data)->format('d/m/Y') }} @ {{ $transferRequest->orario }}</p>
                    <p class="text-gray-600"><span class="font-semibold text-gray-800">{{ $isEn ? 'Passengers' : 'Numero Persone' }}:</span> {{ $transferRequest->numero_persone }}</p>
                </div>
                <div>
                    @if($transferRequest->andata_ritorno)
                        <h3 class="font-bold text-gray-800 mb-2">{{ $isEn ? 'Return Journey' : 'Dettagli Ritorno' }}</h3>
                        <p class="text-gray-600 mb-1"><span class="font-semibold text-gray-800">{{ $isEn ? 'Return Date' : 'Data Ritorno' }}:</span> {{ \Carbon\Carbon::parse($transferRequest->data_ritorno)->format('d/m/Y') }} @ {{ $transferRequest->orario_ritorno }}</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <!-- Finestra Messaggi -->
        <div class="bg-white shadow-md p-6 md:p-8 space-y-6 max-h-[500px] overflow-y-auto border-b">
            ...
            <div class="flex items-start space-x-3 max-w-[85%]">
                <div class="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-sm shadow">
                    {{ strtoupper(substr($transferRequest->nome, 0, 1)) }}
                </div>
                <div class="bg-indigo-50 border border-indigo-100/50 rounded-2xl rounded-tl-none p-4 text-gray-800 shadow-sm text-sm">
                    <div class="flex items-center justify-between mb-1">
                        <span class="font-bold text-indigo-955">{{ $transferRequest->nome }} {{ $transferRequest->cognome }}</span>
                        <span class="text-[10px] text-indigo-500 ml-4">{{ $transferRequest->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <p class="whitespace-pre-line">{{ $transferRequest->messaggio ?: ($isEn ? 'Requested a transfer service.' : 'Richiesta servizio transfer.') }}</p>
                </div>
            </div>

            <!-- Lista dei messaggi del Thread -->
            @foreach($transferRequest->messages as $msg)
                @if($msg->sender === 'admin')
                    <!-- Messaggio dell'Amministratore -->
                    <div class="flex items-start justify-end space-x-3 max-w-[85%] ml-auto">
                        <div class="bg-gray-100 border border-gray-200 rounded-2xl rounded-tr-none p-4 text-gray-800 shadow-sm text-sm">
                            <div class="flex items-center justify-between mb-1">
                                <span class="font-bold text-gray-900 mr-4">{{ $isEn ? 'Support Team' : 'Supporto Clienti' }}</span>
        ...
        <div class="bg-white rounded-b-xl shadow-md p-6 md:p-8">
            ...
            <form action="{{ route('public.transfer.thread.reply', ['token' => $transferRequest->secure_token]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
